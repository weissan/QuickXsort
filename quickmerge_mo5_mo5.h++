...
namespace quickmerge_mo5_mo5 {
	template<typename iter, typename Compare>
	void sort(iter begin, iter end, Compare less) {
		quickmerge::sort_smaller_merge<partition::Stl_partition_mo5_mo5>(begin, end, less);
	}
	template<typename T>
	void sort(std::vector<T> &v) {
		...
		quickmerge::sort_smaller_merge<partition::Stl_partition_mo5_mo5>(begin, end, std::less<T>());
	}
}