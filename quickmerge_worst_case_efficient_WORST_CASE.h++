...
namespace quickmerge_worst_case_efficient_WORST_CASE {
	template<typename iter, typename Compare>
	void sort(iter begin, iter end, Compare less) {
		worst_case_simulation = true;
		quickmerge::sort_worst_case_efficient(begin, end, less);
	...
	void sort(std::vector<T> &v) {
		...
		quickmerge::sort_worst_case_efficient(begin, end, std::less<T>());
		worst_case_simulation = false;
	}
}